...
export function AdminGlobalSettingsPage() {
  ...
  const [taxType, setTaxType] = useState(settings?.tax_type || 'GST');
  const [allowSellerTaxOverride, setAllowSellerTaxOverride] = useState(settings?.allow_seller_tax_override || false);
  const [freeShippingThreshold, setFreeShippingThreshold] = useState(settings?.free_shipping_threshold || 99.00);
  const [platformFulfillmentEnabled, setPlatformFulfillmentEnabled] = useState(settings?.platform_fulfillment_enabled || true);
  const [standardDeliveryDays, setStandardDeliveryDays] = useState(settings?.standard_delivery_days || '2-5');
  const [expressDeliveryDays, setExpressDeliveryDays] = useState(settings?.express_delivery_days || '1-2');
  const [deliveryTrackingEnabled, setDeliveryTrackingEnabled] = useState(settings?.delivery_tracking_enabled || true);
